...
import static com.qmetry.qaf.automation.support.perfecto.DeviceUtils.getBaseBundle;
...
public class CloudUtils {
    ...
    public static DevicesHttpClient getHttpClient() {
        return new DevicesHttpClient(getHostName(), getCredentials(DeviceUtils.getDesiredDeviceCapabilities()));
    }

    public static DesiredCapabilities addDeviceProperties(DesiredCapabilities desiredCapabilities) {

        if (!DeviceUtils.isDevice(desiredCapabilities))
        ...
        DeviceUtils.setActualDeviceCapabilities(desiredCapabilities.asMap());
        ConsoleUtils.logWarningBlocks("ALL DEVICE PROPERTIES SET: " + desiredCapabilities.toString());
        return desiredCapabilities;
    }
    ...
    public static List<ParameterValue> convertMapToParameters(Map<String, ?> mapValues) {
        List<ParameterValue> inputParameters = new LinkedList<>();
        inputParameters.add(new ParameterValue("availableTo", getCredentials(DeviceUtils.getDesiredDeviceCapabilities()).getUser()));

        mapValues.forEach((k, v) -> {
            if ("deviceName".equals(k)) k = "deviceId";
            if ("platformName".equals(k)) k = "os";
            if (new ArrayList<>(Arrays.asList("model", "os", "platformName", "deviceName", "platformVersion", "deviceId", "inUse")).contains(k))
    ...
    public static boolean isDeviceAvailable() {
        ...
        try {
            List<DeviceResult> deviceList = getDeviceList(convertMapToParameters(DeviceUtils.getDesiredDeviceCapabilities().asMap()));
    ...
    public static Credentials getCredentials(DesiredCapabilities dcaps) {
        return new Credentials(dcaps.getCapability("user").toString(), dcaps.getCapability("password").toString());
    }
